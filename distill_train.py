...
class JointDistillationLoss(nn.Module):
    ...
    def __init__(self, dist_ce_wt=5, hard_ce_wt=2, temp=1):
        ...
        # https://github.com/huggingface/transformers/tree/main/examples/research_projects/distillationtransformers/examples/research_projects/distillation at main · huggingface/transformers
    ...
    def forward(self, s_logits, s_ams, t_ams, true_labels, is_train=False):

        tau = self.temp if is_train else 1

        # t_probs = F.softmax(t_logits/tau, dim=-1)
        dist_loss = F.cross_entropy(s_ams, t_ams) * tau**2
...
# dataset for finetuning distilbert with distillation
class DistillTuneDistilBERTDataset(Dataset):

    def __init__(self, tok_dset, t_logits, t_attns):
        ...
        self.attentions = t_attns

        assert len(self.labels) == len(self.soft_labels)
        assert len(self.labels) == len(self.attentions)
    ...
    def __getitem__(self, idx):
        ...
        bert_attentions = torch.FloatTensor(self.attentions[idx])

        fwd_inp = {'input_ids': inp, 'attention_mask': am}

        # labels = {'t_logits': bert_logits, 'label': self.labels[idx]}

        labels = {'t_logits': bert_logits,
                  't_ams': bert_attentions, 'label': self.labels[idx]}
...
def distill_epoch_loop(model, dl, loss_func, device='cpu', optimizer=None):
   ...
   # set training or validation mode
    ...
    for i, (inputs, labels) in enumerate(tqdm(dl, desc='train' if do_train else 'eval')):
        ...
        out = model(**inputs, output_attentions=True)  # TODO: out[logit]
        # loss = loss_func(out['logits'], labels['t_logits'], labels['label'], is_train=do_train)
        loss = loss_func(out['logits'], out.attentions[-1],
                         labels['t_ams'], labels['label'], is_train=do_train)
...
def main():
    ...
    bert_logit_data, bert_attn_data = get_bert_logits(DSET_NAME)

    # initialize datasets that contain (input, attention_mask, label)

    train_data = DistillTuneDistilBERTDataset(
        cola_tok['train'], bert_logit_data['train'], bert_attn_data['train'])

    val_data = DistillTuneDistilBERTDataset(
        cola_tok['validation'], bert_logit_data['validation'], bert_attn_data['validation'])
    ...
    #    'checkpoints/distilbert_distfinetune_original-2023_06_06-18_20.pth'))
    ...
    # run_name = "distilbert_distfinetune_original"
    run_name = "distilbert_distfinetune_attention_bert"
    ...
    val_m_dl = DataLoader(val_data, batch_size=hparams.bsz, shuffle=False)
    ...
    with SummaryWriter(log_dir=log_dir) as log_writer:
        ...
        for epoch in range(hparams.n_epochs):
            ...
            tr_loss, tr_pred, tr_y = distill_epoch_loop(
            ...
            with torch.no_grad():
                ...
                v_m_loss, v_m_pred, v_m_y = distill_epoch_loop(
        ...
        log_writer.add_hparams(
            hparams._dict(), {'res/best_val_acc': best_val_acc})
